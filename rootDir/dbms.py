...
class dbmanager():
    ...
    def initTable(self, table):
        ...
        if self.isjustCreated(self.database[table]+".csv"):
            ...
            df = pandas.DataFrame(csvhandler)
            df.to_csv(self.database[table]+'.csv', header=self.headers[table], index=False)
    ...
    def readTable(self, table):
        try:
            csvhandler = pandas.read_csv(self.database[table]+'.csv', header = 0)
    ...
    def displayCourseStatistics(self, courseID):
        ...
        lst = list()
        for ind in df.index:
            lst.append(root.grade(float(df['marks'][ind])))
        #Histogram
        plt.hist(lst)
